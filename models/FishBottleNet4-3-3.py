...
class FishNet(chainer.Chain):
    ...
    def __init__(self, classes=10, ch0=16,
        ...
        with self.init_scope():
            ...
            self.t1 = Module(ch0 * 4, ch0 * 2, mains, 1, nobias, **dic)
            self.t2 = Module(ch0 * 8, ch0 * 4, mains, 1, nobias, **dic)
            self.t3 = Module(ch0 * 16, ch0 * 16, bottoms, 1, nobias, **dic)
            # body
            self.b2 = Module(ch0 * 32, ch0 * 4, mains, 1, nobias, **dic)
            self.b1 = Module(ch0 * 24, ch0 * 3, mains, 1, nobias, **dic)
            self.b0 = Module(ch0 * 16, ch0 * 2, mains, 1, nobias, **dic)
            # head
            self.h1 = Module(ch0 * 20, ch0 * 5, mains, 1, nobias, **dic)
            self.h2 = Module(ch0 * 36, ch0 * 9, mains, 1, nobias, **dic)
            self.h3 = Module(ch0 * 36, ch0 * 9, lasts, 1, nobias, **dic)
            # linear
...
def model(classes):
    ...
    return FishNet(classes, ch0, firsts, mains, bottoms, lasts, nobias, X=X)
